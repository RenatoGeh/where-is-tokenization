import torch, transformers, tqdm, scipy, Levenshtein
from transformers import AutoTokenizer, AutoModelForCausalLM
import random, math, sys, heapq, multiprocessing, pickle, itertools, os, time
import matplotlib.pyplot as plt, seaborn as sns, pandas as pd, numpy as np, ipdb
import models, utils

PREFIX="/space/renatolg/tokens"
sys.setrecursionlimit(100_000)
torch.multiprocessing.set_start_method("spawn", force=True)

#MAR_BATCH_LIST = [(32, 76), (16, 110), (8, 170), (4, 260), (2, 403)]
#MAR_BATCH_LIST = [(64, 50000), (32, 60000), (16, 70000), (8, 80000), (4, 90000), (2, 100000)]
...
class MDD:
    ...
    def bb(self, tokenizer: AutoTokenizer, model: AutoModelForCausalLM, k: int = 10,
        ...
        if bound_tok is None: bound_tok = self.shortest(add_eos=False)
        ...
        while (len(Q) > 0) and time_budget > 0:
            ...
            for ll, path_c, (t, c) in zip(LL, P, mdd._ch):
                ...
                if t == eos_token or c is None:
                    ...
                    else: heapq.heappush(top_k, (ll.item(), path))
    ...
    def dot(self, path: str = None, tokenizer: AutoTokenizer = None):
        text = "strict digraph {\n"""
...
def cache_vocab(tokenizer: AutoTokenizer, cache_file: str = None, cache: dict = None,
    ...
    if exists:
        ...
        if utils.is_llama_tokenizer(tokenizer): f_const = _llama_vocab_const
        elif utils.is_gemma_tokenizer(tokenizer): f_const = _llama_vocab_const
        else: f_const = _vocab_const
        V = dict(utils.parallelize(utils.partial(f_const, tokenizer), tokenizer.get_vocab().keys(),
                                   desc="Constructing vocabulary"))
        # V = dict(map(utils.partial(f_const, tokenizer), tokenizer.get_vocab().keys()))
